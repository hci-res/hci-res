...
Our final paper prototype features two tasks -- searching for events and creating an event. The searching for events task includes entering a date, time, and location. The search result then comes up and users can decide whether to declare their attendance, interest, or non-attendance to events. The creating an event task include dragging and dropping a pin on the desired event location on a map which transitions to a screen where the user needs to input an emoji, the event’s name, date, time, and description. 
